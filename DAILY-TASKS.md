...
- [ ] **Owner:** the signed test still gets `{"ok":false}`. Re-paste the current `Code.gs` (it now returns a setup code: `setup_secret_missing` / `bad_signature` / `setup_tab_missing` …), run `setupWebsiteLeads`, and Deploy → Manage → **New version**. GA4: mark `lead_received` as a key event.
