...
- [x] Vercel env vars `LEAD_SHEETS_WEBHOOK_URL` + `LEAD_SHEETS_SECRET` set (Production, via CLI). Web app is public (returns `{"ok":false}` to unsigned requests).
- [x] **Sheet connected and verified 2026-09-24.** Web app `AKfycby_aOw6…/exec` (a second deployment; the first URL still serves old code, so ignore it). Live contact form → row saved → `generate_lead` + `lead_received`. Three "TEST — delete me" rows were written during verification.
- [ ] GA4: mark `lead_received` as a key event.
- [x] **Skip-to-WhatsApp on the 5-step concept wizard**, on every step, carrying the answers so far (`wizard_skip_whatsapp`). The wizard hides the mobile bar, so it had no way out.
- [x] **Every "Request quote"/"Get quote" lands on the form** (`contact#quote`, 78 header links + mobile bar + legacy chat), not ~2,200px above it on mobile.
- [x] **Sticky contact nudge** on every page except contact/wizard/ROI/privacy (`buildNudge` in main.v2.js). Copy by page type (guide/tool/service/other). Shows at 35% scroll or first calculator input; dismissible per session. Desktop: bottom bar clear of the FAB. Mobile: strip above `.mact`; skipped on mobile calculators, whose sheet already has WhatsApp. Events `nudge_view` / `nudge_dismiss`.
...
- [x] **Spokes expanded 291–335 → 786–846 words**, using the repo's own verified engineering constants rather than padding. Ammonia page gained condenser derate at Pakistani ambient (2.0%/K MT, 2.7%/K LT, 60% floor), buoyancy factors (1.45 at −25 °C), strip-curtain F=0.10, the glycol approach/pump-power trade, and charge-inventory reasoning. Rack page gained the part-load staging argument, air vs evaporative vs water condenser selection at 46–50 °C, and how three suction groups map to chill/frozen/dock. CDU page gained **brochure p38** verbatim (standard or custom sizes, with or without insulated floor, the 15-application list) plus insulated-floor guidance from the documented floor U of 0.7 W/m²K and the on-grade 18 °C vs above-grade ambient boundary rule, and IEC 60335-2-89 A2L charge limits. Each closes with links into the calculator suite — real internal linking into `tools/`, not filler.
- [x] **`united-snacks-wall-cdus`** added from brochure p37, cropped below the title text — a row of wall-mounted condensing units, one per room, which is the CDU argument in one photograph.
- [x] **Fixed a second inheritance bug the expansion exposed:** all three spokes still carried the CA page's Tool 8 pitch ("Add CA-store equipment scope — nitrogen, ethylene removal, GAC analysis") and deep-linked the cost calculator to `#cmd=apple;sp=ca_store_apple;ca=1` — an **apple CA store preset** on three refrigeration pages. Copy rewritten per page and the preset dropped; the genuine CA page keeps its preset.

- [x] **Clone-hazard checklist added to CLAUDE.md** (task algorithm C). Two separate rounds of fixes were spent on content inherited from `ca-stores.html` — FAQs, `FAQPage` schema, an applications grid, `Service` `serviceType`/`audienceType`/`additionalProperty`, and a cost-calculator deep link preset to an apple CA store. The visible copy looked correct each time, which is why it survived review. The checklist names the six greps that catch it.
- [x] **Verified on live production, not just locally.** All three new pages return 200 with the expanded content deployed and zero CA presets; `/solutions` shows the ten cards in the specified order. **Vercel scripts: 0 tags on home, cold-stores and contact** — the removal held across both deploys, so the earlier warning that the dashboard integrations would re-inject them was wrong. No dashboard action is needed.
...
- [x] **Concept wizard was below the fold on every desktop.** The 2026-08-10 fold fix was written inside `@media (max-width: 720px)`, so only phones got it — at 1280×800 the hero still ran **589px, 74% of the viewport**, putting question one's first answer at **891px**, unreachable without scrolling. The same failure that was fixed on mobile, left standing on the screens where the tool gets demoed. Now compressed in three tiers, because the constraint is *height*, not width: ≥721px wide, then `max-height: 820px` (1366×768 laptops), then `max-height: 740px` (resized windows, drops the lead exactly as mobile does). Hero 608→381px on desktop; question and the first two rows of answers clear the fold at 1440×900, 1280×800, 1366×768, 1440×700, iPad, iPhone SE and Android. Measured, not eyeballed.
...
- [x] **NOT bots — GA4 same-day processing lag. Corrected 2026-08-04 after the owner pushed back.** An earlier version of this entry called 54 unattributed sessions "near-certainly bots". Wrong. Proof: `(not set)` was **45 sessions today, 0 on Aug 3, 1 on Aug 2** — bots do not vanish from historical days. `google / organic` engagement was **25/32 (78%) on Aug 3 vs 1/34 (3%) today** for the same source on the same site. `google / cpc` grew **12 → 27 sessions between two queries an hour apart**, i.e. data still landing. And `(data not available)` is literally GA4's own label for data still being processed. **Never draw engagement or bounce conclusions from a same-day GA4 query** — attribution and engagement metrics are not yet computed. The Vercel bot filter is still worth adding for other reasons, but it was not the cause here.
- [!] **Consequence: the "100% mobile bounce" figure used to justify the panel-page fix was an artefact.** The fix stands anyway — it rested on DOM measurements (calculator 4.2 screens down, first price 21 screens down, no visible above-fold CTA), not on GA4. But the bounce framing was unsound and should not be repeated.
- [ ] **Re-check the funnel on complete data.** The zero-submit findings are probably real — `form_start` 5 processed while `form_submit` stayed 0, and both process alike — but confirm against a settled day before treating the leak as quantified.
...
- [x] **GTM conversion pipeline LIVE and verified in the published container.** 13 variables, 7 triggers, `Ads – Conversion Linker` + `Ads – Conv – Lead (All Channels)` (ID `18369062794`, label `zvz9CLWf8dscEIrPhrdE`, value `{{Lookup – Lead Value}}`, currency USD, Transaction ID `{{dlv.click_ref}}`, trigger `CE – Lead Any` regex). Verified by fetching `gtm.js?id=GTM-WBNZLVC7`: `awct`=1, conversion ID + label present, regex present, zero PKR leftovers.
- [x] **Conversion action #1 `IF – Lead (All Channels)`** created — Primary, Submit lead form, $30, Count One, 90-day window, account-default goal. Two Google-auto-created placeholders neutralised (one Removed, one demoted to Secondary) — both had been Primary, steering the PMax campaign at a goal with no working signal.
- [x] **Time zone stays (GMT-04:00) Eastern — cannot be changed.** Accepted, not fixed. Mitigations applied: (a) **no dayparting** — ADS-PLAN §B4 now says run all hours and let Smart Bidding handle time-of-day, since schedules would be entered in ET and drift an hour at each DST change while Pakistan has none; (b) **compare 7/28-day windows, not single days**, because an Ads day runs 09:00→09:00 PKT and will never align with GA4 daily figures; (c) **offline uploads (§A12) stamp Pakistani local time with an explicit `+05:00` offset**, which Google accepts. Net impact is small — the day boundary lands at Pakistani business open, so the workday sits inside one Ads day and the budget resets as the office opens.
- [ ] **6 reporting conversion actions + tags** (`· Form`/`· WhatsApp`/`· Phone`/`· Email`/`Micro · Tool Result`/`Micro · Deep Engagement`) — all Secondary, optional. Skipped deliberately: the single Primary is what bids, per-channel values already vary via the lookup table, and GA4 has the channel split. ~20 min when granularity is wanted.
- [ ] **`CE – Lead Email` trigger had the wrong filter** (`dlv.ud.email contains email` → should be `dlv.lead_channel equals email`). Harmless while no tag uses it; fix before building `Ads – Conv – Lead Email`.
- [ ] **Enhanced conversions not wired** — the *Include user-provided data* field is absent from the Ads conversion tag template because there is no Google tag **inside** the container (ours is hard-coded on the page). Clean fix: move the `AW-18369062794` config off the 77 pages into a GTM Google tag, which also clears the "No Google tag found in this container" warning and gives `UPD – Lead` a home. Repo-side change, worth ~+10% measured conversions on form leads only.
...
| Date | Commit | Items |
|------|--------|-------|
| 2026-09-02 | _pending_ | **Google Ads Tag Diagnostics: "Tag quality: Urgent" → cleared 3 action items** (GTM-SETUP §A14). (1) *Security settings blocking measurement* — `vercel.json` CSP only allowed `www.googletagmanager.com` + `www.google-analytics.com`, so the `AW-18369062794` destination's Google Ads conversion library (`www.googleadservices.com`) and its remarketing/conversion endpoints (`googleads.g.doubleclick.net`, `td.doubleclick.net`) were blocked by `default-src 'self'` on **every page**; `frame-src` also omitted `www.googletagmanager.com`, blocking the GTM `<noscript>` iframe — the only iframe the site ships. Added the full tag-platform allowlist across `script-src` / `frame-src` / `connect-src` / `style-src`, incl. `tagmanager.google.com` so GTM Preview mode works on the live domain. (2) *Additional domains detected* — `izharfoster.vercel.app` was serving the whole site at HTTP 200 with the same GTM/GA4/Ads tag (duplicate content **and** duplicate tagging); added a host-conditioned 308 → `izharfoster.com` as the first redirect, plus `X-Robots-Tag: noindex` on any `*.vercel.app` host so preview deploys can't be indexed either. Preview URLs (`izharfoster-<hash>-*.vercel.app`) don't match the exact host, so previews still serve. `www` was already 308ing correctly. (3) *Pages not tagged* — no code change: a repo-wide sweep of all 86 HTML files found only 3 untagged, all deliberate (GSC verification stub, `noindex` styleguide, `noindex`+`Disallow`d client reports). Verified: JSON parses, 21-assertion CSP check passes (required hosts present, pre-existing `unpkg`/fonts/`object-src 'none'` intact), diff is exactly 27 insertions / 1 deletion with nothing reformatted, and `verify-ads-tracking.mjs` still 46/46. **UI follow-ups (cannot be done in code):** in Ads → Tag quality → Configure your domains, decline/remove `izharfoster.vercel.app`; then re-check Diagnostics 24–48 h after deploy (Tag quality is computed on a lag). |
| 2026-06-20 | _pending_ | **Surfaced 5 orphaned pages into navigation** (GP§13 internal linking; fixes pages that shipped into `sitemap.xml` but were unreachable by browsing). Walk-in Cold Rooms, Blast Freezers, and Marine & Boat Cold Storage existed as service pages with near-zero inbound links and **0 presence on `/solutions`**; the two boat blog posts (`solar-cold-storage-fishing-boats-pakistan`, `stop-losing-your-catch-boat-cold-storage`) had **0 presence on `/blog`**. Added: 3 product cards to the `solutions.html` prod-grid (now 9 cards, marine uses `images/projects/marine-fish-on-ice`), 2 blog cards to `blog.html` (top of grid, newest-first, marine images), and a **site-wide footer sweep** adding Walk-in / Blast / Marine to the footer Solutions list on all 63 HTML files (46 sub-depth `../services/` + 17 root-depth `services/`, perl in-place). All pages 200, JSON-LD intact, grid screenshot-verified. |
| 2026-06-20 | _pending_ | **Cold Map export pins — Afghanistan, Kazakhstan, Bangladesh** (`projects.html`; demand-driven from the Apr–Jul 2026 inquiry list — #11 Almaty/Kazakhstan panels DONE, #23 Sherazi/Afghanistan DONE, #69 Mustafiz/Bangladesh 40-room cold-store enquiry open). Widened the inline SVG Cold Map viewBox 800→920 to make room east (Bangladesh) and north (Kazakhstan, Afghanistan); added 3 land silhouettes, 3 dashed export-route lines from Lahore, and 3 hollow "export reach" pins matching the existing GCC pin style (per user decision to treat as export reach). **Honesty guard:** pin sub-labels say "quotation issued" / "enquiry" not "delivered" — accurate to the inquiry list, avoids the fabricated-locality risk flagged in the KSA work. Updated intro copy, legend ("Export reach · GCC & beyond"), disclaimer ("Hollow pins" not "GCC pins"), and stat tile (4 GCC cities → 7 export markets). KSA service-page map left GCC-focused (on-topic for that page). Screenshot-verified placement + label collisions fixed; 3 JSON-LD blocks parse, balanced `<g>` tags. |
